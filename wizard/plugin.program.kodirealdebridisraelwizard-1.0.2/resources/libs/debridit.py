...
ORDER          = ['fenrd', 'fenad', 'mando', 'premiumizer', 'serenrd', 'serenpm', 'rurlrd', 'rurlpm','shadowrd', 'myaccountsrd', 'myaccountspm', 'myaccountsad', 'urlrd', 'urlpm']
...
DEBRIDID = {
	...
	'mando': {
		...
		'saved'    : 'realmando',
		'path'     : os.path.join(ADDONS, 'plugin.video.mando'),
		'icon'     : os.path.join(ADDONS, 'plugin.video.mando', 'icon.png'),
		'fanart'   : os.path.join(ADDONS, 'plugin.video.mando', 'fanart.jpg'),
		'file'     : os.path.join(REALFOLD, 'mando_debrid'),
		'settings' : os.path.join(ADDOND, 'plugin.video.mando', 'settings.xml'),
		'default'  : 'rd.client_id',
		'data'     : ['rd.auth', 'rd.refresh', 'rd.client_id', 'rd.secret', 'rd.expiry'],
		'activate' : 'RunPlugin(plugin://plugin.video.mando?mode=138&url=www)'},
    'serenrd': {
        'name'     : 'Seren RD',
