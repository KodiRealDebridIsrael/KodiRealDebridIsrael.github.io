...
# Send email function
def send_email_build_installation_log():
    from resources.libs.common import logging
    logging.log("Custom KODI_RD_ISRAEL LOG: Starting send_email_build_installation_log() function")
    ...
    try:
        ...
        # Get the IP address - Make a request to the ipify.org API to get the public IP address
        with urllib.request.urlopen("https://api.ipify.org?format=json") as response:
            ip_address = json.loads(response.read())['ip']
        ...
        # Compose the message
        mail_content = (
            f"Installation Time: {current_time}\n"
            f"Hostname: {hostname}\n"
        ...
        msg['Subject'] = 'Kodi + Real Debrid Israel Build - Installation Log'
        ...
        server.quit()
        logging.log("Custom KODI_RD_ISRAEL LOG: send_email_build_installation_log() SUCCESS")

    except Exception as e:
        # log the error message into wizard.log
        logging.log("Custom KODI_RD_ISRAEL LOG: An error occurred during sending email of build installation log. Error:")
        logging.log(e)
...
def wipe():
    ...
    # Send email for build installation log
    send_email_build_installation_log()
