    ...
    class firstRun(xbmcgui.WindowXMLDialog):
        ...
        def onInit(self):
            ...
            self.controllist     = [self.trakt, self.debrid, #self.login,
                                    self.sources, self.profiles, self.playercore, self.advanced, self.myvideos, self.pvr,
            ...
            self.controlsettings = ['keeptrakt', 'keepdebrid', #'keeplogin',
                                    'keepsources', 'keepprofiles', 'keepplayercore', 'keepadvanced', 'keepmyvideos', 'keeppvr',
...
def firstRun():
    class MyWindow(xbmcgui.WindowXMLDialog):
        def __init__(self, *args, **kwargs):
            ...
            self.msg   = "ברוכים הבאים ל- %s.\n\nלחץ על [B]'תפריט בילדים' [/B]לבחירת בילד להתקנה.\n\nתודה על בחירתכם ב- %s." % (ADDONTITLE, ADDONTITLE)
