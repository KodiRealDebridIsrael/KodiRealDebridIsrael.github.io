...
def whiteList(do):
    ...
    if   do == 'edit':
        ...
        for folder in sorted(fold2, key = lambda x: x):
            ...
            addonfolds.append(foldername)
        selected = []; choice = 0
        tempaddonnames = ["[B]-- לחץ כאן כדי לשמור ולהמשיך --[/B]"] + addonnames
        ...
        while not choice in [-1, 0]:
            choice = DIALOG.select("%s: !בחר אילו הרחבות ברצונך להוסיף לרשימת השמירה" % ADDONTITLE, tempaddonnames)
            if choice == -1: break
            elif choice == 0: break
            else:
                choice2 = (choice-1)
        ...
        list = whiteList('read')
        if len(list) > 0:
            msg = "[B]זו רשימת ההרחבות השמורות שלך (כולל התלויות) שלא יוסרו בעת התקנה חדשה או עדכון בילד.[/B][CR][CR]"
            for item in list:
                try: name, id, fold = item
                except Exception as e: log(str(e))
        ...
        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]האם הינך בטוח שברצונך לנקות את רשימת ההרחבות השמורות? לא ניתן לבטל תהליך זה.[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]ניקוי[/COLOR][/B]", nolabel="[B][COLOR red]ביטול[/COLOR][/B]"):
        ...
        try:
            for root, dirs, files in os.walk(PACKAGES):
                ...
                if file_count > 0:
                    ...
                    if over: yes=1
                    else: yes=DIALOG.yesno("[COLOR %s]Delete Package Files" % COLOR2, "[COLOR %s]%s[/COLOR] files found / [COLOR %s]%s[/COLOR] in size.Do you want to delete them?[/COLOR]" % (COLOR1, str(file_count), COLOR1, size), nolabel='[B][COLOR red]Don\'t Clear[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Clear Packages[/COLOR][/B]')
                    if yes:
...
def killxbmc(over=None):
    if over:
        ...
        choice = DIALOG.yesno('תפריט יציאה', '[COLOR %s]הינך עומד לסגור את קודי. האם להמשיך?[/COLOR]' % COLOR2, nolabel='[B][COLOR white]לא, ביטול[/COLOR][/B]',yeslabel='[B][COLOR white]יציאה מקודי[/COLOR][/B]')
    if choice == 1:
        log("Force Closing Kodi: Platform[%s]" % str(platform()), LOGNOTICE)
        os.system('start TASKKILL /im ELEMEN~1.EXE /f');os.system('tskill ELEMEN~1.EXE')
        os._exit(1)
