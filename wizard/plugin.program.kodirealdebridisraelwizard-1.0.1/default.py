                ...
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        ...
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove the addon_data for:[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, plugin), yeslabel="[B][COLOR springgreen]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"):
...
def net_tools(view=None):
    addFile ('Speed Tester' ,'speedtestM', icon=ICONSPEED, themeit=THEME1)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def saveMenu():
    ...
    addDir ('שמורים Trakt נתוני',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
    addDir ('שמורים Debrid נתוני',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
    #addDir ('נתוני התחברות שמורים',               'login',                icon=ICONLOGIN, themeit=THEME1)
    addFile('ייבוא נתונים שמורים',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME1)
    addFile('יצוא נתונים שמורים',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME1)
    addFile('- Click to toggle settings -', '', themeit=THEME3)
    addFile('Trakt שמירת חשבון: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME1)
    addFile('Debrid שמירת חשבון: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME1)
    #addFile('שמירת נתוני התחברות: %s' % login.replace('true',on).replace('false',off)                  ,'togglesetting', 'keeplogin',      icon=ICONLOGIN, themeit=THEME1)
    addFile('Sources.xml שמירת: %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME1)
    addFile('Profiles.xml שמירת: %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME1)
    addFile('playercorefactory.xml שמירת: %s' % playercore.replace('true', on).replace('false', off), 'togglesetting', 'keepplayercore', icon=ICONSAVE, themeit=THEME1)
    addFile('Advancedsettings.xml שמירת: %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME1)
    addFile('MyVideos.db שמירת: %s' % myvideos.replace('true',on).replace('false',off) ,'togglesetting', 'keepmyvideos',   icon=ICONSAVE,  themeit=THEME1)
    addFile('pvr.iptvsimple שמירת: %s' % pvr.replace('true',on).replace('false',off) ,'togglesetting', 'keeppvr',   icon=ICONSAVE,  themeit=THEME1)
    addFile('Favourites.xml שמירת: %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME1)
    addFile('Super Favourites שמירת: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME1)
    addFile('שמירת מאגרי ריפו: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME1)
    addFile('שמירת הרחבות מותקנות: %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME1)
    if whitelist == 'true':
        ...
        addFile('ייבוא רשימת הרחבות לשמירה',      'whitelist', 'import', icon=ICONSAVE,  themeit=THEME1)
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        ...
        if yes_pressed:
            ...
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]Please Wait' % (COLOR2, wiz.checkBuild(name,'color'), name, wiz.checkBuild(name,'version')))
            ...
            DP.update(0, '%s Please Wait' % title)
            percent, errors, error = extract.all(lib,HOME,DP, title=title)
...
def thirdPartyInstall(name, url):
    ...
    zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR]Please Wait' % (COLOR2, COLOR1, name))
    lib=os.path.join(PACKAGES, '%s.zip' % zipname)
    try: xbmcvfs.delete(lib)
    except: pass
    downloader.download(url, lib, DP, name)
...
def apkInstaller(apk, url):
    ...
    if wiz.platform() == 'android':
        ...
        if KODIV >= 18:
            ...
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR]Please Wait' % (COLOR2, COLOR1, display))
...
def manageSaveData(do):
    if do == 'import':
        ...
        zipf.close()
        if source == mybuilds:
            DIALOG.ok(ADDONTITLE, "[COLOR %s]Save data has been backed up to:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, tempzip))
        else:
            try:
                xbmcvfs.copy(tempzip, os.path.join(source, 'SaveData.zip'))
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Save data has been backed up to:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, os.path.join(source, 'SaveData.zip')))
            except:
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Save data has been backed up to:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, tempzip))
...
def freshStart(install=None, over=False):
    ...
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]האם ברצונך להוריד ולהתקין את [COLOR %s]%s[/COLOR]?"% ( COLOR2, COLOR1, install), nolabel = '[B] [COLOR red]ביטול[/COLOR][/B]', yeslabel = '[B] [COLOR springgreen]התקנה[/COLOR] [/B]' )
    ...
    if yes_pressed:
        ...
        if KEEPPVR == 'true':
            EXCLUDES.append('pvr.iptvsimple')
        ...
        for root, dirs, files in os.walk(xbmcPath,topdown=True):
            ...
            for name in files:
                ...
                elif name == 'MyVideos119.db' and fold[-1] == 'Database' and KEEPMYVIDEOS == 'true':  wiz.log("Keep MyVideos.db: %s" % os.path.join(root, name), LOGNOTICE)
                elif name == 'pvr.iptvsimple' and fold[-1] == 'addon_data' and KEEPPVR == 'true':  wiz.log("Keep pvr.iptvsimple: %s" % os.path.join(root, name), LOGNOTICE)
...
elif mode=='fixaddonupdate' : wiz.kodi19Fix()
